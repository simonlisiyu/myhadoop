...
import com.winhong.gzszyy.commons.redis.{RedisWriter, RedisReader}
...
object PredicateForInputWords {
  ...
  def main(args: Array[String]) {
    val inputStr = RedisReader.getUserPredicate("0")
    val userId = inputStr.split("-")(0)
    val keywordList = inputStr.split("-")(0).split(" ").toList
//    val keywordList = List("体重","太阳病39")
//    println("result: "+PredicateByInputWords(keywordList))
//    RedisWriter.writeUserPredicate(userId,PredicateByInputWords(keywordList))
    ...
    conf.setAppName("PredicateALS").setMaster("local[2]")
    ...
    println("----------user input keywords--------")
    val inputWords = sc.parallelize(keywordList).map(x => {
      val wId = keywordMap.getOrElse(x,"0")
      (wId,x)
    })
    inputWords.foreach(println)
    val inputRatings = inputWords.map { line =>
      Rating(0, line._1.toInt, 1.0)
    ...
    val reduceSample = sc.parallelize(userLogList).map(x => {
      ...
      println("0: "+splits(0)+",1: "+splits(2)+",2: "+splits(3))
      (splits(0)+"-"+splits(2),splits(3).toInt)
    }).reduceByKey(_ + _)
    ...
    val day = getNowDate().split("-")(2)
    if(!RedisReader.getPredicateUpdateDayForSparkCache.equals(day)){
      textSample.unpersist()
      RedisWriter.updatePredicateDayForSparkCache(day)
    }
    ...
    val training = textSample.filter(x => x._1 < 6)
      .values
      .union(inputRatings) // 注意 ratings 是 (Int,Rating) ，取 value 即可
    ...
    */
    ...
    val myRatedMovieIds = inputWords.map(z => z._1.toInt).collect()
    val candidates = sc.parallelize(idWordMap.keys.filter(!myRatedMovieIds.contains(_)).map(x => x.toInt).toSeq)
    ...
    RedisWriter.writeUserPredicate(userId,keywordStr)
    //结束
    sc.stop()
  }
  ...
  def PredicateByInputWords(keywordList :List[String]) :String = {
    Logger.getLogger("org.apache.spark").setLevel(Level.WARN)
    Logger.getLogger("org.eclipse.jetty.server").setLevel(Level.OFF)
