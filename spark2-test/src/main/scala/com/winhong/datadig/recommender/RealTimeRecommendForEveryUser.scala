...
/**
  ...
  */
object RealTimeRecommendForEveryUser {
...
//  val recommendForUsersMap = scala.collection.mutable.Map(0 -> "word")
  ...
  def main(args: Array[String]) {
    ...
    conf.setAppName("UserSearchALS").setMaster("local[3]")
...
//    keywordMap.foreach(println)
    ...
    val userLogList = recoSer.getUserLogsByDate(getBefore30Date(),getTomorrowDate()).toArray()
//    userLogList.foreach(println)
...
//    textSample.map(_._2.product).distinct().foreach(println)
    ...
    val realTimeLog = recoSer.getUserLogsByDate(getYesterDayDate(),getTomorrowDate()).toArray()
    realTimeLog.foreach(println)
    val realTimeLogReduce = sc.parallelize(realTimeLog).map(x => {
      val splits = x.toString.split("\t")
      println("User=====0: "+splits(0)+",1: "+splits(1)+",2: "+splits(2))
      (splits(0)+"-"+splits(1),splits(2).toInt)
    }).reduceByKey(_ + _)

    val realTimeLogData = realTimeLogReduce.map(x => {
      val splits = x._1.split("-")
      val wordId = keywordMap.getOrElse(splits(1),"0")
      (Math.random()*10000%10 , Rating(splits(0).toInt, wordId.toInt, x._2.toDouble))
    })

    val uIdRDD = realTimeLogData.map(_._2.user).distinct().toArray()
    val uIdKeywordRDD = realTimeLogData.map(x => (x._2.user, x._2.product)).toArray()
    ...
    val training = textSample.filter(x => x._1 < 6)
      .values
      .union(realTimeLogData.map(_._2)) // 注意 ratings 是 (Int,Rating) ，取 value 即可
    ...
    */
    ...
    uIdRDD.foreach { uId =>
      ...
      val uKeywordIdRDD = realTimeLogData.map(y => {
        if(y._1.equals(uId))
          y._2
        else 0
      }).collect()

      val candidates = sc.parallelize(idWordMap.keys.filter(!uKeywordIdRDD.contains(_)).map(x => x.toInt).toSeq)
      val recommendations = bestModel.get.predict(candidates.map((uId, _)))
        .collect()
        .sortBy(_.rating)
        .take(10)
      ...
      RedisWriter.writeUserPredicate(uId+"",keywordStr)
    }
